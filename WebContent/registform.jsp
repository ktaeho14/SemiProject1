...
<script type="text/javascript">
	...
	/*비밀번호 유효성검사 ajax*/
	$(function(){
		$("input[name=mypw]").blur(function(){
		
			var mypw = $("input[name=mypw]").val();
			...
			$.ajax({
				...
				success: function(obj){
					...
					if (obj.id != null) {
						...
						/*아이디 중복아닐때*/
						if(myid.trim()==""||myid==null){
							$(".idck").html("아이디는 5~12자야 하며, 숫자/영문소문자로만 이루어집니다.");
							$(".idck").css("color", "red");
						} else if (!idRegExp.test(myid)){
							$(".idck").html("아이디는 5~12자야 하며, 숫자/영문소문자로만 이루어집니다.");
							$(".idck").css("color", "red");
						} else {
							$(".idck").html("사용가능한 아이디입니다.");
							$(".idck").css("color", "#19ce60");
							$("input[name=myid]").attr("title","y");
						}
					}
				},
				error:function(){
