...
<%
	...
	List<complainDto> list2 = (List<complainDto>)request.getAttribute("list2");
...
%>
		<tr>
			<td><%=list2.get(i).getComplain_boardno() %></td>  
			<td><a href="<%=request.getContextPath()%>/complain.do?command=complain_detail&complain_boardno=<%=list2.get(i).getComplain_boardno()%>"><%=list2.get(i).getComplain_title() %></a></td>  
			<td><%=list2.get(i).getComplain_name() %></td>  
			<td><%=list2.get(i).getComplain_regdate() %></td>  
...
%>		
	...
	<tr>
		<td colspan="5" align="right">
			<input type="button"  id="insert" value="글쓰기" onclick="location.href='<%= request.getContextPath()%>/complain.do?command=complain_insertform'"
